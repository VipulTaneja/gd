...
interface StaffReviewListProps {
...
export function StaffReviewList({ reviews }: StaffReviewListProps) {
  if (reviews.length === 0) {
    return <p className="text-sm text-muted-foreground">{staffCopy.noReviews}</p>;
  }

  return (
    <div className="space-y-3">
      {reviews.map((review) => (
        <SoftCard key={review.id} accent="gold" className="p-4">
