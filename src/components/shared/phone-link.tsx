...
interface PhoneLinkProps {
  ...
  /** Hide the phone number text and show only the icon (still links to WhatsApp) */
  iconOnly?: boolean;
}
  ...
  if (!href) {
    return (
      <span className={cn("inline-flex items-center gap-1.5", className)}>
        {!iconOnly && <WhatsAppIcon className="text-muted-foreground/50" />}
        {!iconOnly && label}
      </span>
    );
  }
  ...
  return (
    <a
      ...
      className={cn(
        ...
        "hover:text-[#128C7E] hover:underline hover:underline-offset-4",
        className,
      )}
